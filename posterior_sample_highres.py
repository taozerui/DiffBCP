...
def posterior_sample(cfg):
    ...
    # dataset = Image.open(data_config.root).resize(
    #     (data_config.image_size, data_config.image_size), Image.LANCZOS)
    dataset = Image.open(data_config.root)
    dataset = np.array(dataset)[None, :, :, :]
    dataset = torch.from_numpy(dataset).float().permute(0,3,1,2) / 255.0
    num_test_images = 1
    img_size = dataset.shape[2:]

    # generate masks
    if data_config.mask_name.startswith('irr'):
        mask = np.load('./data/putt_irregular_mask_area50-70_brush2-6.npy')
        mask = torch.from_numpy(mask).float()
        mask = mask.permute(0,3,1,2)
    else:
        obs_ratio = float(data_config.mask_name.split('_')[-1])
        mask = torch.zeros(img_size).reshape(-1)
        obs_num = int(obs_ratio * mask.numel())
        obs_idx = torch.randperm(mask.numel())[:obs_num]
        mask[obs_idx] = 1.0
    ...
    for i in range(num_test_images):
        ...
        for j in tqdm(range(cfg.num_runs)):
            ...
            for xz in ['x', 'z']:
                samples = all_samples[xz].to(device)
                ...
                for name, metric in metrics.items():
                    log[xz][name+"_sample"].append(metric(sample, inv_transform(ref_img)).item())
                log[xz]["consistency_sample"].append(torch.norm(operator.forward(transform(sample)) - y_n).item())
                ...
                if len(samples) > 1:
                    ...
                    for name, metric in metrics.items():
                        log[xz][name+"_mean"].append(metric(mean, inv_transform(ref_img)).item())
                    log[xz]["consistency_mean"].append(torch.norm(operator.forward(transform(mean)) - y_n).item())
